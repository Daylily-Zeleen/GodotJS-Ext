define("jsb.editor.codegen", ["require", "exports"], function (require, exports) {
    ...
    const jsb = require("godot.lib.api").jsb;
    ...
    const RemappedPrimitiveTypeNames = {
    ...
    const BindableVariantTypes = (function () {
        const types = [];
    ...
    const VariantTypeNames = (function () {
    ...
    function get_primitive_type_name_as_input(type) {
        ...
        switch (type) {
            case godot.Variant.Type.TYPE_PACKED_COLOR_ARRAY:
                return join_type_name(primitive_name, get_js_array_type_name(VariantTypeNames.get(godot.Variant.Type.TYPE_COLOR)));
            case godot.Variant.Type.TYPE_PACKED_VECTOR2_ARRAY:
                return join_type_name(primitive_name, get_js_array_type_name(VariantTypeNames.get(godot.Variant.Type.TYPE_VECTOR2)));
            case godot.Variant.Type.TYPE_PACKED_VECTOR3_ARRAY:
                return join_type_name(primitive_name, get_js_array_type_name(VariantTypeNames.get(godot.Variant.Type.TYPE_VECTOR3)));
    ...
    class AbstractWriter {
        ...
        property_(name, static_property = false) {
    ...
    class DocCommentHelper {
        ...
        static write(writer, description, newline) {
            ...
            let lines = Array.isArray(description)
                ? description
                : this.get_simplified_description(typeof description === "string" ? Description.forAny(description).text : description.text)
                    .replace("\r\n", "\n")
                    .split("\n");
    ...
    class IndentWriter extends BufferingWriter {
        _never_collapse;
        _indent_first_line;
        constructor(base, always_multiline = false, indent_first_line = true) {
            super(base);
    ...
    const annotation_types = {
        ...
        RPCConfig: godot.GDictionary.create({
            type: DescriptorType.ObjectLiteral,
            properties: {
                mode: {
                    type: DescriptorType.Godot,
    ...
    class ClassWriter extends IndentWriter {
        ...
        head() {
            const params = this._generic_parameters
                ? `<${Object.entries(this._generic_parameters)
                    .map(([name, p]) => `${name}${p.extends ? ` extends ${p.extends}` : ""}${p.default ? ` = ${p.default}` : ""}`)
                    .join(", ")}>`
                : "";
            let class_extends = "";
        ...
        property_(name_or_getset_info, static_property = false) {
            ...
            const line = (line) => this.line(property_override?.(line) ?? line);
    ...
    class PropertyWriter extends BufferingWriter {
        ...
        _key;
        _static_property;
        constructor(base, name, static_property = false, concatenate_first_line = false) {
            super(base);
            this._concatenate_first_line = concatenate_first_line;
            this._key = name;
    ...
    class TypeDB {
        ...
        make_classname(class_name, internal) {
        ...
        make_typename(info, used_as_input, non_nullable) {
            const null_prefix = !non_nullable &&
            ...
            if (info.class_name.length === 0 || info.class_name.includes(",")) {
                ...
                if (typeof variant_type_name !== "undefined") {
                    if (info.type === godot.Variant.Type.TYPE_ARRAY &&
        ...
        is_empty_default_value(value) {
            ...
            if (typeof value === "object") {
                const maybe_is_empty = value.is_empty;
        ...
        make_literal_value(value) {
            ...
            if (value.type >= godot.Variant.Type.TYPE_PACKED_BYTE_ARRAY &&
    ...
    class TSDCodeGen {
        ...
        _splitter;
        _types;
        _use_project_settings;
        constructor(out_dir, use_project_settings) {
            this._split_index = 0;
        ...
        emit_utility(utility_func) {
            const global_doc = this._types.find_doc("@GlobalScope"); // TODO
            const cg = this.split();
        ...
        emit_global(global_obj) {
            const cg = this.split();
            const doc = this._types.find_doc("@GlobalScope"); // TODO
        ...
        emit_godot_class(cg, cls, singleton_mode) {
            try {
                ...
                for (const method_info of rpc_methods) {
                    rpc_interface_writer.property_(method_info.name, `(${this._types.make_args(method_info)}) => ${this._types.make_return(method_info)}`);
                }
    ...
    class SceneTSDCodeGen {
        ...
        make_scene_path(scene_path, include_filename = true) {
            const relative_path = (include_filename ? scene_path.replace(/\.t?scn$/i, ".nodes.gen.ts") : scene_path.replace(/\/[^\/]+$/, "")).replace(/^res:\/\/?/, "");
            if (typeof this._out_dir !== "string" || this._out_dir.length == 0) {
        ...
        emit_scene_node_types(scene_path) {
            try {
                ...
                const children = helper.get_scene_nodes(scene_path)?.proxy();
    ...
    class ResourceTSDCodeGen {
        ...
        constructor(out_dir, resource_paths) {
            ...
            const script_extensions = new Array(length);
        ...
        make_resource_path(resource_path, include_filename = true) {
            const relative_path = (include_filename ? resource_path + ".gen.ts" : resource_path.replace(/\/[^\/]+$/, "")).replace(/^res:\/\/?/, "");
            if (typeof this._out_dir !== "string" || this._out_dir.length == 0) {
                return relative_path;
            }
            return this._out_dir.endsWith("/") ? this._out_dir + relative_path : this._out_dir + "/" + relative_path;
        ...
        get_script_rpc_info(resource_path) {
        ...
        emit_script_rpc_types(module, resource_path) {
        ...
        emit_resource_type(resource_path) {
            try {
                const helper = godot.GodotJSEditorHelper;
                const descriptor = helper.get_resource_type_descriptor(resource_path)?.proxy();
