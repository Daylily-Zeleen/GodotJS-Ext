...
4. Publishing the GitHub Release starts another CI run through the `release: published` event.
5. After that release-triggered CI run succeeds, a separate `workflow_run` workflow downloads its artifacts and attaches the V8 and QuickJS-NG packages to the release.

The build and publish stages are intentionally separate. If a matrix build, especially Windows V8 dependency download, needs to be rerun, the successful CI run can finish without relying on downstream jobs that were already skipped in the original run.
...
- **`Upload Assets` is skipped**: confirm the release-triggered CI run completed successfully. The upload workflow runs after that CI run through `workflow_run` and downloads artifacts from the completed run.
