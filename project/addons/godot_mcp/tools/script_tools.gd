...
func validate_script(args: Dictionary) -> Dictionary:
	...
	# ResourceLoader.load() gets a script with proper context (path, dependency
	# resolution), avoiding GDScript.new() orphan-instance issues:
	#   - preload() with relative paths can be resolved
	#   - class_name won't trigger "hides a global class" errors
	# CACHE_MODE_REPLACE ensures fresh file content is used.
	...
	if script == null:
		...
		return {
			...
			&"message": "Failed to load script." \
					+ (" Details: " + "; ".join(errors) if errors.size() > 0 \
					else " Check Godot console for details.")
		}

	# reload() on the already-loaded script triggers parsing with full context.
	# Returns OK on success, or a compile error code on failure.
	...
	if err != OK:
		...
		return {
			...
			&"errors": errors,
			&"error_code": err,
			&"message": "Script has errors." \
					+ (" Details: " + "; ".join(errors) if errors.size() > 0 \
					else " " + error_string(err) + ". Check Godot console for details.")
		}
