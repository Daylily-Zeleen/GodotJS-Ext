...
private:
	static void constructor(const v8::FunctionCallbackInfo<v8::Value> &info) {
		...
		jsb_throw(isolate, jsb_format("unexpected 'new.target', you may be instantiating a script class which is not exported as default. class: %s [native: %s (%d)]", impl::Helper::to_string_opt(isolate, new_target->Get(context, jsb_name(environment, name))), class_name, class_id));
	}
