...
#		define JSB_V8_VERSION_STR V8_S(JSB_MAJOR_VERSION) "." V8_S(JSB_MINOR_VERSION)
...
public:
	...
	bool on_received(const unsigned char *p_buf, size_t p_len) {
		...
		if (lws_is_final_fragment(wsi_)) {
			...
			if (is_binary) {
				JSB_DEBUGGER_LOG(Verbose, "receive binary message: %d", recv_buffer_->get_position());
			} else {
				const String msg = String::utf8((const char *)recv_buffer_->get_data_array().ptr(), recv_buffer_->get_position());
				JSB_DEBUGGER_LOG(Verbose, "receive text message: %s", msg);
			}
			...
			case LWS_CALLBACK_HTTP: {
				...
				if (uri == "/json" || uri == "/json/list") {
					...
					constexpr static char kJsonVersionFormat[] =
							"{"
							"    \"Browser\": \"" JSB_MODULE_NAME_STRING "/" JSB_V8_VERSION_STR "\","
							"    \"Protocol-Version\" : \"1.1\","
							"    \"User-Agent\" : \"" JSB_MODULE_NAME_STRING "/" JSB_V8_VERSION_STR "\","
