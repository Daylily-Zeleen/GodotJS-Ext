...
let JSWorkerParent = ENVIRONMENT_IS_PTHREAD ? undefined : null;
function getJSWorkerParent() {
...
function jsbb_create_transfer_marker(transfer_index) {
    const marker = {
...
function jsbb_is_transfer_marker(value) {
...
function jsbb_is_worker_message(value) {
    return typeof value === "object" && value?.["__jsbMessage"] || false;
}
...
class jsbb_Engine {
    ...
    MapSetEntry(map_sp, key_sp, value_sp) {
        const map = this._stack.GetValue(map_sp);
        if (!(map instanceof Map)) {
            return -1;
        }
    ...
    PromiseResolverGetPromise(resolver_sp) {
        const resolver = this._stack.GetValue(resolver_sp);
        return this._stack.Push(resolver.promise);
    }
    PromiseResolverResolve(resolver_sp, value_sp) {
        try {
            const resolver = this._stack.GetValue(resolver_sp);
    ...
    PromiseResolverReject(resolver_sp, value_sp) {
        try {
            const resolver = this._stack.GetValue(resolver_sp);
...
class _jsbb_ {
    static queue_worker_message(message) {
    ...
    static init(interop) {
        ...
        try {
            ...
            for (const key of Object.keys(this.interop)) {
                jsbb_console.trace("define jsbi.interop", key, typeof this.interop[key]);
            }
            for (const key of Object.keys(this.wasmop)) {
    ...
    static is_godot_binding_object(value) {
        return typeof value === "object" && value !== null && typeof value[jsbb_opaque] === "number";
    }
    static is_plain_object(value) {
        const proto = Object.getPrototypeOf(value);
        return proto === Object.prototype || proto === null;
    }
    static get_or_add_native_transfer_index(engine, engine_opaque, pthread_id, transfer_id, context, value) {
        ...
        if (!Number.isInteger(native_index) || native_index < 0) {
            ...
            if (native_index === -1 /* jsbb_WorkerTransferErrorCode.ObjectNotInTransferList */) {
                throw new Error(`A Godot Object was passed that was not in the transfer list. (${descriptor})`);
            }
            if (native_index === -5 /* jsbb_WorkerTransferErrorCode.VariantConversionFailed */) {
                throw new Error(`Failed to convert JS value to Variant for worker transfer. (${descriptor})`);
            }
            if (native_index === -6 /* jsbb_WorkerTransferErrorCode.VariantCloneFailed */) {
                throw new Error(`Failed to clone Godot Variant for worker transfer. (${descriptor})`);
            }
            if (native_index === -2 /* jsbb_WorkerTransferErrorCode.EnvMissing */
                || native_index === -3 /* jsbb_WorkerTransferErrorCode.IsolateMissing */
                || native_index === -4 /* jsbb_WorkerTransferErrorCode.NonObjectValue */
                || native_index === -7 /* jsbb_WorkerTransferErrorCode.TransferAppendFailed */) {
    ...
    static try_get_or_add_native_transfer_index(engine, engine_opaque, pthread_id, transfer_id, context, value) {
        const existing_index = context.transfer_index_by_object.get(value);
        if (typeof existing_index === "number") {
            return existing_index;
        }
    ...
    static create_transfer_encode_context() {
        return {
            native_transfers_used: false,
            transfer_index_by_object: new Map(),
            seen: new WeakMap(),
        };
    }
    static seed_native_transfer_index_map(engine, engine_opaque, pthread_id, transfer_id, context, transfer_list) {
        ...
        const iterable = transfer_list[Symbol.iterator];
    ...
    static extract_message_variants(engine, engine_opaque, pthread_id, transfer_id, context, value) {
    ...
    static post_message_to_worker(pthread_id, data, transfer_id) {
        try {
    ...
    static install_pthread_worker_listener_hook() {
    ...
    static dispatch_direct_worker_message(data, sender_pthread_id) {
    ...
    static RegisterWorkerOwner(pthread_id, _engine_opaque, worker_owner_sp) {
        ...
        const worker_owner = engine.stack.GetValue(worker_owner_sp);
    ...
    static PostMessage(pthread_id, engine_opaque, data_sp, transfer_id, native_transfers_stack_position) {
        const engine = this.engine;
        if (!engine) {
            return -1 /* jsbb_PostMessageResult.Failed */;
        }
        ...
        try {
            ...
            const rewritten = this.extract_message_variants(engine, engine_opaque, pthread_id, transfer_id, transfer_context, message);
            ...
            if (ENVIRONMENT_IS_PTHREAD) {
                ...
                return typeof outbound_transfer_id === "number"
                    ? 0 /* jsbb_PostMessageResult.Success */
                    : 1 /* jsbb_PostMessageResult.NativeTransfersUnused */;
            }
    ...
    static flush_pending_worker_messages() {
    ...
    static FlushPendingWorkerMessages() {
        this.flush_pending_worker_messages();
    }
...
_jsbb_.evalResult = undefined;
_jsbb_._pending_worker_messages = [];
_jsbb_._worker_owner_cache_by_pthread = new Map();
