...
void OnPostGCCallback(v8::Isolate *isolate, v8::GCType type, v8::GCCallbackFlags flags) {
	JSB_LOG(VeryVerbose, "v8 gc time %dms type:%d flags:%d", OS::get_singleton() ? Time::get_singleton()->get_ticks_msec() - gc_ticks : -1, type, flags);
}
#endif
...
Variant Environment::call_script_method(ScriptClassID p_script_class_id, NativeObjectID p_object_id, const StringName &p_method, const Variant **p_argv, int p_argc, GDExtensionCallError &r_error) {
	...
	if (!is_caller_thread()) {
		...
		JSB_LOG(Error, "can not call script method from a different thread (env_thread=%s caller_thread=%s object_id=%s object_instance_id=%s script_instance_ptr=%s script_env_thread=%s method=%s object_type=%s node_path=%s)", String::num_uint64((uint64_t)thread_id_), String::num_uint64((uint64_t)caller_thread_id), String::num_uint64(*p_object_id), owner_instance_id, script_instance_ptr, script_env_thread, String(p_method), object_type, node_path);
...
void Environment::transfer_in_bind(const v8::Local<v8::Context> &p_context, const TransferData &p_data) {
	...
	jsb_checkf(ScriptInstance::get_script_instance(instance) == nullptr,
			"Transferred godot object should not have a script instance, it may associate with another env: %d",
			instance->to_string());
