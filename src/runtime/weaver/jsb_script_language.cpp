...
GodotJSScriptLanguage::~GodotJSScriptLanguage() {
	...
	jsb::impl::GlobalInitialize::shutdown();
}
...
std::shared_ptr<jsb::Environment> GodotJSScriptLanguage::create_shadow_environment() {
	...
	JSB_LOG(Log, "creating a shadow Environment on thread %d for %s [env %s]", ThreadEx::get_caller_id(), jsb_typename(GodotJSScript), (uintptr_t)env->id());
	env->init();
	{
		std::lock_guard shadow_lock(shadow_mutex_);
