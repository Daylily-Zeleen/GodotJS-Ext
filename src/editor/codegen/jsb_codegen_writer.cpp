...
String DocCommentHelper::replace_markup_content(const String &p_text, int p_from_pos, const String &p_markup, const String &p_rep) {
    ...
    if (index >= 0) {
        return replace_markup_content(
                ...
                index + p_rep.length(), p_markup, p_rep);
    }
    return p_text;
}
...
InterfaceWriter *Writer::interface_(const String &p_name, const HashMap<String, GenericParameter> *p_generics,
...
ClassWriter::ClassWriter(Writer *p_base, const ClassEmitOptions &p_options)
        : IndentWriter(p_base, true), options_(p_options), types_(p_base->get_types()),
          docs_(types_ != nullptr ? types_->find_doc(options_.name, options_.original_name.is_empty() ? options_.name : options_.original_name) : nullptr) {
...
void EnumWriter::element_(const String &p_name, int64_t p_value) {
    ...
    DocCommentHelper::write(*this,
            doc != nullptr && doc->constants.has(p_name) ? doc->constants[p_name] : String(), separator_line_);
    separator_line_ = true;
