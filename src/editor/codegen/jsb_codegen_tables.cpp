...
void build_animation_library(TypeMutation &t) {
	...
	add_override(t, "rename_animation", chain({ make_mutate_parameter_type("name", "AnimationName"), make_mutate_parameter_type("newname", "AnimationName") }));
	add_override(t, "has_animation", chain({ make_mutate_parameter_type("name", "AnimationName") }));
	...
	add_override(t, "animation_renamed", lit({ "readonly " + m("animation_renamed") + ": Signal<(name: StringName, " + p("to_name") + ") => void>" }));
...
void build_animation_mixer(TypeMutation &t) {
	...
	add_override(t, "add_animation_library", chain({ make_mutate_template("Name extends keyof LibraryMap"), make_mutate_parameter_type("name", "Name"), make_mutate_parameter_type("library", "LibraryMap[Name]") }));
	add_override(t, "remove_animation_library", chain({ make_mutate_template("Name extends keyof LibraryMap"), make_mutate_parameter_type("name", "Name") }));
	add_override(t, "rename_animation_library", chain({
	...
	add_override(t, "has_animation_library", chain({ make_mutate_template("Name extends keyof LibraryMap"), make_mutate_parameter_type("name", "Name") }));
	add_override(t, "get_animation_library", chain({ make_mutate_template("Name extends keyof LibraryMap"), make_mutate_parameter_type("name", "Name"), make_mutate_return_type("LibraryMap[Name]") }));
	add_override(t, "get_animation_library_list", mut(make_mutate_return_type("keyof LibraryMap extends GAny ? GArray<keyof LibraryMap> : GArray")));
	add_override(t, "has_animation", chain({ make_mutate_template("Name extends " + static_mixer_path), make_mutate_parameter_type("name", "Name") }));
	add_override(t, "get_animation", chain({ make_mutate_template("Name extends " + static_mixer_path), make_mutate_parameter_type("name", "Name"), make_mutate_return_type("ResolveAnimationMixerPath<LibraryMap, Name>") }));
}

void build_animation_player(TypeMutation &t) {
	const String path = "StaticAnimationMixerPath<LibraryMap>";
	add_override(t, "animation_set_next", chain({ make_mutate_parameter_type("animation_from", path), make_mutate_parameter_type("animation_to", path) }));
	add_override(t, "animation_get_next", chain({ make_mutate_parameter_type("animation_from", path), make_mutate_return_type(path) }));
	add_override(t, "set_blend_time", chain({ make_mutate_parameter_type("animation_from", path), make_mutate_parameter_type("animation_to", path) }));
	add_override(t, "get_blend_time", chain({ make_mutate_parameter_type("animation_from", path), make_mutate_parameter_type("animation_to", path) }));
	...
	add_override(t, "current_animation", lit({ "get " + m("current_animation") + "(): " + path, "set " + m("current_animation") + "(value: " + path + ")" }));
	add_override(t, "assigned_animation", lit({ "get " + m("assigned_animation") + "(): " + path, "set " + m("assigned_animation") + "(value: " + path + ")" }));
	add_override(t, "autoplay", lit({ "get autoplay(): " + path, "set autoplay(value: " + path + ")" }));
	add_override(t, "current_animation_changed", lit({ "readonly " + m("current_animation_changed") + ": Signal<(name: " + path + ") => void>" }));
	add_override(t, "animation_changed", lit({ "readonly " + m("current_animation_changed") + ": Signal<(" + p("old_name") + ": " + path + ", " + p("new_name") + ": " + path + ") => void>" }));
}
...
void build_callable(TypeMutation &t) {
	...
	add_override(t, "bind", chain({ make_mutate_template("A extends any[]"), make_mutate_parameter_type("...varargs", "A"), make_mutate_return_type("Callable<BindRight<T, A>>") }));
...
void build_camera_feed(TypeMutation &t) {
	...
	add_override(t, "formats", lit({ "get formats(): GArray<CameraFeed.FeedFormat>", "set formats(value: GArray<CameraFeed.FeedFormat>)" }));
}

void build_editor_undo_redo_manager(TypeMutation &t) {
	const String object_class = c("Object");
	add_override(t, "add_do_method", lit({ m("add_do_method") + "<T extends " + object_class + ", M extends GodotNames<T>>(object: T, method: M, ...args: ResolveGodotNameParameters<T, M>): void" }));
	add_override(t, "add_undo_method", lit({ m("add_undo_method") + "<T extends " + object_class + ", M extends GodotNames<T>>(object: T, method: M, ...args: ResolveGodotNameParameters<T, M>): void" }));
	add_override(t, "add_do_property", lit({ m("add_do_property") + "<T extends " + object_class + ", P extends GodotNames<T>>(object: T, property: P, value: ResolveGodotNameValue<T, P>): void" }));
...
void append_garray_overrides(TypeMutation &t) {
	...
	add_override(t, "bsearch_custom", chain({ make_mutate_parameter_type("value", elem), make_mutate_parameter_type("func", "Callable<(a: GArrayElement<T>, b: GArrayElement<T>) => boolean>") }));
...
void build_garray(TypeMutation &t) {
	t.generic_parameters.push_back({ "T", gp(String(kGodotAnyType) + " | " + kGodotAnyType + "[]", String(kGodotAnyType) + " | " + kGodotAnyType + "[]") });
	t.intro = {
...
void append_gdictionary_overrides(TypeMutation &t) {
	...
	add_override(t, "merged", chain({ make_mutate_parameter_type("dictionary", "GDictionary<U>"), make_mutate_return_type("GDictionary<T & U>"), make_mutate_template("U") }));
	add_override(t, "has", mut(make_mutate_parameter_type("key", "keyof T")));
	add_override(t, "has_all", mut(make_mutate_parameter_type("keys", "keyof T extends GAny ? GArray<keyof T> : GArray")));
	add_override(t, "find_key", chain({ make_mutate_parameter_type("value", "T[keyof T]"), make_mutate_return_type("keyof T") }));
	...
	add_override(t, "values", mut(make_mutate_return_type("UndefinedToNull<T[keyof T]> extends GAny ? GArray<UndefinedToNull<T[keyof T]>> : GArray")));
	add_override(t, "duplicate", mut(make_mutate_return_type("GDictionary<T>")));
	add_override(t, "get", chain({ make_mutate_parameter_type("key", "K"), make_mutate_return_type("UndefinedToNull<T[K]>"), make_mutate_template("K extends keyof T") }));
	add_override(t, "get_or_add", chain({ make_mutate_parameter_type("key", "K"), make_mutate_return_type("UndefinedToNull<T[K]>"), make_mutate_parameter_type("default_", "T[K]"), make_mutate_template("K extends keyof T") }));
	add_override(t, "set", chain({ make_mutate_parameter_type("key", "K"), make_mutate_parameter_type("value", "T[K]"), make_mutate_template("K extends keyof T") }));
}
...
void build_input(TypeMutation &t) {
	...
	add_override(t, "get_axis", chain({ make_mutate_parameter_type("negative_action", "InputActionName"), make_mutate_parameter_type("positive_action", "InputActionName") }));
	add_override(t, "get_vector", chain({ make_mutate_parameter_type("negative_x", "InputActionName"), make_mutate_parameter_type("positive_x", "InputActionName"), make_mutate_parameter_type("negative_y", "InputActionName"), make_mutate_parameter_type("positive_y", "InputActionName") }));
...
void build_node(TypeMutation &t) {
	...
	add_override(t, "get_node", lit({ m("get_node") + "<Path extends StaticNodePath<Map>, Default = never>(path: Path): ResolveNodePath<Map, Path, Default>" }));
	add_override(t, "get_node_or_null", lit({
												m("get_node_or_null") + "<Path extends StaticNodePath<Map, undefined | Node>, Default = null>(path: Path): null | "
	...
	const String object_class = c("Object");
	add_override(t, "rpc", lit({ m("rpc") + "<Method extends GodotRPCNames<this>>(method: Method, ...varargs: ResolveGodotRPCParameters<this, Method>): Error" }));
	add_override(t, "rpc_id", lit({ m("rpc_id") + "<Method extends GodotRPCNames<this>>(" + p("peer_id") + ": int64, method: Method, ...varargs: "
																										   "ResolveGodotRPCParameters<this, Method>): Error" }));
}

void build_object(TypeMutation &t) {
	add_override(t, "call", lit({ "call<M extends GodotNames<this>>(method: M, ...args: ResolveGodotNameParameters<this, NoInfer<M>>): "
								  "ResolveGodotReturnType<this, NoInfer<M>>" }));
	add_override(t, "call_deferred", lit({ m("call_deferred") + "<M extends GodotNames<this>>(method: M, ...args: ResolveGodotNameParameters<this, NoInfer<M>>): void" }));
	add_override(t, "set_deferred", lit({ m("set_deferred") + "<P extends GodotNames<this>>(property: P, value: ResolveGodotNameValue<this,  NoInfer<P>>): void" }));
	add_override(t, "callv", lit({ "callv<M extends GodotNames<this>>(method: M, argArray: GArray<ResolveGodotNameParameters<this, NoInfer<M>>>): "
								   "ResolveGodotReturnType<this, NoInfer<M>>" }));
...
void build_packed_to_array(TypeMutation &t, const char *p_js_type, const char *p_element_type, const char *p_class_name, bool p_is_byte_buffer) {
	if (p_is_byte_buffer) {
		t.intro = {
...
void build_resource_loader(TypeMutation &t) {
	add_override(t, "load", lit({
									"static load<Path extends keyof ResourceTypes>(path: Path, " + p("type_hint") + "?: string /* = \"\" */, " + p("cache_mode") + "?: ResourceLoader.CacheMode /* = 1 */): ResourceTypes[Path]",
									"static load(path: string, " + p("type_hint") + "?: string /* = \"\" */, " + p("cache_mode") + "?: ResourceLoader.CacheMode /* = 1 */): Resource",
							}));
	add_override(t, "load_threaded_get", lit({
