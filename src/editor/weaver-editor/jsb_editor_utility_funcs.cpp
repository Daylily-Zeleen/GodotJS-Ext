...
#include "runtime/bridge/jsb_editor_utility_funcs.h"
...
void build_builtin_class_constant(v8::Isolate *isolate, const v8::Local<v8::Context> &context, const api_tool::ApiBuiltInClassConstantInfo &api_builtin_class_constant, const v8::Local<v8::Object> &builtin_class_constant_obj) {
	...
	switch (api_builtin_class_constant.type) {
		case Variant::BOOL:
			set_field(isolate, context, builtin_class_constant_obj, "value", (bool)constant_value);
			break;
		case Variant::INT:
			set_field(isolate, context, builtin_class_constant_obj, "value", (int64_t)constant_value);
			break;
		case Variant::FLOAT:
			set_field(isolate, context, builtin_class_constant_obj, "value", (double)constant_value);
			break;
		default:
			break;
	}
...
static void _get_global_constants(const v8::FunctionCallbackInfo<v8::Value> &info) {
	...
	for (const StringName &enum_name : enum_names) {
		...
		for (const auto &api_enum : enum_info->values) {
			JSB_HANDLE_SCOPE(isolate);
			const v8::Local<v8::String> enum_name = impl::Helper::new_string(isolate, internal::NamingUtil::get_enum_value_name(api_enum.name));
			const v8::Local<v8::Value> enum_value = impl::Helper::new_integer(isolate, api_enum.value);
			values_obj->Set(context, enum_name, enum_value).Check();
		}
...
static void _expose_editor_impl(v8::Isolate *isolate, v8::Local<v8::Context> context, v8::Local<v8::Object> jsb_obj) {
...
void EditorUtilityFuncs_register_impl() {
	EditorUtilityFuncs::set_expose_impl(&_expose_editor_impl);
}
} //namespace jsb
#else
namespace jsb {
void EditorUtilityFuncs_register_impl() {
	// Editor utility functions disabled for this platform/target: keep the runtime stub.
}
} //namespace jsb
