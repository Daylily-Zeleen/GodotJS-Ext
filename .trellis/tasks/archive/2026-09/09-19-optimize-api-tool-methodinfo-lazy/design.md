...
- 上表为**布局测算**（探针实测尺寸 × json 计数）。实施后复核：尺寸已由 `api_tool_types.h` 末尾的 7 条 `static_assert` 在**真实类型**上强制（非仅探针镜像），构建 `EXIT=0` 即尺寸成立；计数来自 json（权威输入）。运行时堆分配器级实测未单独做——布局测算 + 编译期强制是当前最高置信证据。
