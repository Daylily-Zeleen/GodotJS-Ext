...
> 分支：`feature/static-bindings` · 状态：**v8（已实施，P0-P5 主体完成；收尾见本任务子任务）**
...
│   └─ misc/build/static_binding_codegen.py   │
...
if env["static_binding"]:
    ...
    # 代码生成：python misc/build/static_binding_codegen.py \
...
- 位置：`misc/build/static_binding_codegen.py`（纯标准库：json + 文本模板，零依赖）
