...
- [ ] 0.2 抽取运算符覆盖矩阵（脚本留 `.agent_tmp/`，不入库）：
  ...
  - **在 R6（去 String）完成后重新抽取**，得到最终覆盖集合；实测规模：236 JS 方法（一元 50 + 二元 186）、186 表、344 可选组合。
...
- [ ] 3.3 `jsb_codegen_writer.cpp` `ClassWriter::operator_`（`:526`）重写：一元 `name(): R`；二元 `name(right: U): R`，`right_type == Variant::NIL` 时 `U = kGodotAnyType`（`GAny`；2026-09-19 修正，原为 `"Variant | null"`——`Variant` 在 typings 里不是类型）；删 `static` 前缀与 `left` 形参；`separator_line_ = true` 保留。
...
- [ ] 5.3 嵌入 §0.2 产出的覆盖矩阵，实现 design §6 的六层断言：
  1. 形态：全部方法 `typeof value.OP_X === "function"` + `(Class as any).OP_X === undefined`（236）；
  2. 契约：全部可选组合逐个调用不抛 + 返回型别与声明一致（344）；
...
- [ ] 7.5 运算符专项：确认覆盖计数（236 方法 / 344 组合）由测试自身打印并核对；完整性守卫**做一次负向验证**（临时从期望集合删一项 → 测试必须报失败 → 还原）。
